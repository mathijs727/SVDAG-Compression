...
void optionsKeyCallback(GLFWwindow* win, int key, int scancode, int action, int mods)
{
    ...
    if (key == GLFW_KEY_ESCAPE)
        finish = true;
    if (key == GLFW_KEY_1)
        renderer->decDrawLevel();
    if (key == GLFW_KEY_2)
        renderer->incDrawLevel();
    if (key == GLFW_KEY_3)
        renderer->decGPUTraversalMaxIters(10);
    if (key == GLFW_KEY_4)
        renderer->incGPUTraversalMaxIters(10);
    if (key == GLFW_KEY_5)
        renderer->decPixelTolerance();
    if (key == GLFW_KEY_6)
        renderer->incPixelTolerance();
    if (key == GLFW_KEY_R)
        renderer->nextViewerRenderMode();
    if (key == GLFW_KEY_O)
    ...
    if (key == GLFW_KEY_F)
        cam->toggleCamController();

    if (key == GLFW_KEY_F1)
        renderer->selectRenderMode(OctreeDDARenderer::RenderMode::AO);
    if (key == GLFW_KEY_F2)
        renderer->selectRenderMode(OctreeDDARenderer::RenderMode::DEPTH);
    if (key == GLFW_KEY_F3)
        renderer->selectRenderMode(OctreeDDARenderer::RenderMode::SHADOW);
    if (key == GLFW_KEY_F4)
        renderer->selectRenderMode(OctreeDDARenderer::RenderMode::VIEWER);
}
...
bool loadFile(std::string inputFile)
{
    ...
    if (ext == "svdag" || ext == "SVDAG") {
        encoded_octree = std::make_unique<EncodedSVDAG>();
        if (!encoded_octree->load(inputFile))
            incorrectFile = true;
    } else if (ext == "ussvdag" || ext == "USSVDAG") {
        encoded_octree = std::make_unique<EncodedUSSVDAG>();
        if (!encoded_octree->load(inputFile))
            incorrectFile = true;
    } else if (ext == "ssvdag" || ext == "SSVDAG"
        || ext == "esvdag" || ext == "ESVDAG") {
        encoded_octree = std::make_unique<EncodedSSVDAG>();
        if (!encoded_octree->load(inputFile))
            incorrectFile = true;
    } else
        incorrectFile = true;
...
void handleImgui()
{
    ...
    camPosInput[0] = camPos[0];
    camPosInput[1] = camPos[1];
    camPosInput[2] = camPos[2];

    const sl::point3f& lightPos = renderer->getLightPos();
    float lightPosInput[] { 0, 0, 0 };
    lightPosInput[0] = lightPos[0];
    lightPosInput[1] = lightPos[1];
    lightPosInput[2] = lightPos[2];
...
int main(int argc, char** argv)
{
    ...
    renderer = std::make_unique<OctreeDDARenderer>(encoded_octree.get());
