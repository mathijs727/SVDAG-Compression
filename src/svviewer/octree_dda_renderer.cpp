...
void OctreeDDARenderer::uploadData(bool init)
{
    ...
    if (_octreeFormat == SVO) {
        ...
        if ((svo->getDataSize() / 16) > maxTBOTexels)
            printf("\t- WARNING! ");
        else
            printf("\t- ");
        ...
        glBindTexture(GL_TEXTURE_BUFFER, _glTex[0]);
        glBindBuffer(GL_TEXTURE_BUFFER, _glBuf[0]);
        if ((dag->getDataSize() / 16) > maxTBOTexels)
            printf("\t- WARNING! ");
        else
            printf("\t- ");
        ...
        glBindTexture(GL_TEXTURE_BUFFER, _glTex[0]);
        glBindBuffer(GL_TEXTURE_BUFFER, _glBuf[0]);
        if ((sdag1->getDataSize() / 16) > maxTBOTexels)
            printf("\t- WARNING! ");
        else
            printf("\t- ");
        ...
        glBindTexture(GL_TEXTURE_BUFFER, _glTex[0]);
        glBindBuffer(GL_TEXTURE_BUFFER, _glBuf[0]);
        if ((sdag4->getLeafNodesSize() / 8) > maxTBOTexels)
            printf("\t- WARNING! ");
        else
            printf("\t- ");
        ...
        if (_sdag4UseTex3D) { // as a 3D Texture
            ...
            if ((sdag4->getInnerNodesSize() / 8) > maxTBOTexels)
                printf("\t- WARNING! ");
            else
                printf("\t- ");
...
void OctreeDDARenderer::init()
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (RenderMode(i) == VIEWER)
            _program[i].setDefine("VIEWER_MODE", 1);
        else if (RenderMode(i) == DEPTH)
            _program[i].setDefine("DEPTH_MODE", 1);
        else if (RenderMode(i) == SHADOW)
            _program[i].setDefine("SHADOW_MODE", 1);
        ...
        case SVO:
            _program[i].setDefine("SVO", 1);
            break;
        case SVDAG:
            _program[i].setDefine("SVDAG", 1);
            break;
        case USSVDAG:
            _program[i].setDefine("USSVDAG", 1);
            break;
        case SSVDAG:
            _program[i].setDefine("SSVDAG", 1);
            break;
        default:
            return;
        }
        ...
        if (!_program[i].setAndCompileShader(GLSLProgram::VERTEX_SHADER, _sqr.getDefaultVertexProgram()))
            exit(-1);
        if (!_program[i].loadAndCompileShader(GLSLProgram::FRAGMENT_SHADER, SHADER_FILE))
            exit(-1);
        if (!_program[i].link())
            exit(-1);
...
void OctreeDDARenderer::draw(GLint renderBuffer)
{
    ...
    if (_useMinDepthOptimization && (_selectedRenderMode == VIEWER || _selectedRenderMode == DEPTH)) {
        ...
        glUseProgram(_program[_selectedRenderMode].getGLProgram());
        if (_selectedRenderMode == VIEWER || _selectedRenderMode == DEPTH)
            glUniform1i(_program[_selectedRenderMode].getUniformID("useMinDepthTex"), false);
    }
    ...
    if (_selectedRenderMode == VIEWER) {
        ...
        if (_octreeFormat == SSVDAG)
            glUniform1i(_program[_selectedRenderMode].getUniformID("freqColors"), _freqColors);
    }
...
std::string OctreeDDARenderer::getRenderModeName(RenderMode mode)
{
    ...
    case AO:
        ...
        break;
    default:
        s = "NO_VALID_RENDER_MODE";
    }
    return s;
}

void OctreeDDARenderer::setEncodedOctree(EncodedOctree* eo)
{
    _encodedOctree = eo;
    if (dynamic_cast<EncodedSVO*>(_encodedOctree) != NULL) {
        _octreeFormat = SVO;
